...
class dockbr:
    ...
    def dock(self, mframe):
        ...
        btnprfrncs = Button(statusfrm, width=48, height=48,
                            image=imgprfrncs, bg='white', command=self.win.xtra)
        btnprfrncs.pack(side=LEFT, padx=2, pady=2)
        btnprfrncs.image = imgprfrncs

        # Enable Dock
        imgenb = PhotoImage(file="icon/enable.gif")
        self.btnenb = Button(statusfrm, width=48, height=48, image=imgenb,
                             bg='white', command=self.enable, state=DISABLED)
        self.btnenb.pack(side=LEFT, padx=2, pady=2)
        self.btnenb.image = imgenb

        # Disable Dock
        imgdisbl = PhotoImage(file="icon/disable.gif")
        self.btndisbl = Button(statusfrm, width=48, height=48,
                               image=imgdisbl, bg='white', command=self.disable)
        self.btndisbl.pack(side=LEFT, padx=2, pady=2)
        self.btndisbl.image = imgdisbl

        # Flush Dock
        imgfls = PhotoImage(file="icon/fls.gif")
        btnfls = Button(statusfrm, width=48, height=48,
                        image=imgfls, bg='white', command=firewl.flush)
        ...
        btnsave = Button(statusfrm, width=48, height=48,
                         image=imgsave, bg='white', command=clrld.save)
        btnsave.pack(side=LEFT, padx=2, pady=2)
        btnsave.image = imgsave

        # Restore Dock
        imgres = PhotoImage(file="icon/res.gif")
        btnres = Button(statusfrm, width=48, height=48,
                        image=imgres, bg='white', command=clrld.restore)
        btnres.pack(side=LEFT, padx=2, pady=2)
        btnres.image = imgres

        # Clear Dock
        imgclr = PhotoImage(file="icon/clr.gif")
        btnclr = Button(statusfrm, width=48, height=48,
                        image=imgclr, bg='white', command=self.clear)
        btnclr.pack(side=LEFT, padx=2, pady=2)
        btnclr.image = imgclr

        # Reload Dock
        imgrld = PhotoImage(file="icon/rld.gif")
        btnrld = Button(statusfrm, width=48, height=48,
                        image=imgrld, bg='white', command=self.reload)
        ...
        btnadd = Button(statusfrm, width=48, height=48,
                        image=imgadd, bg='white', command=self.apnd)
        btnadd.pack(side=LEFT, padx=2, pady=2)
        btnadd.image = imgadd

        # Delete Dock
        imgrmv = PhotoImage(file="icon/rmv.gif")
        btnrmv = Button(statusfrm, width=48, height=48,
                        image=imgrmv, bg='white', command=self.delete)
        btnrmv.pack(side=LEFT, padx=2, pady=2)
        btnrmv.image = imgrmv

        # Insert Dock
        imgedt = PhotoImage(file="icon/insert.gif")
        btnedt = Button(statusfrm, width=48, height=48,
                        image=imgedt, bg='white', command=self.inst)
        btnedt.pack(side=LEFT, padx=2, pady=2)
        btnedt.image = imgedt

        # Default Dock
        imgdef = PhotoImage(file="icon/default.gif")
        btndef = Button(statusfrm, width=48, height=48,
                        image=imgdef, bg='white', command=rulesmeth.default)
    ...
    def enable(self):
        if (self.firwlstts == 0):
            ...
            tkinter.messagebox.showinfo(
                "Enabled", "The Firewall is already Enabled.")
    ...
    def disable(self):
        if (self.firwlstts == 1):
            ...
            tkinter.messagebox.showinfo(
                "Disabled", "The Firewall is already Disabled.")
